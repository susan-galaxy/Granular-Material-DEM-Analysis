...
%  Phase Diagram, and Predictive Clustering Map  (REVISION 2)
%  ---------------------------------------------------------------------
%  Improvements over v1:
%   • Two-pass fit: refit only outer (ballistic) shells when clustering
%     indicators trigger (σ²_n>2  OR  T_c/T_w<0.3).
%   • Detect & flag saturated low-φ fits (λ > λ_max_phys); plot as
%     upward-triangle LOWER BOUNDS with arrows in Fig 1.
%   • Fig 3 replaced: dual-axis "theory breakdown" plot — coincident
%     divergence of λ_DEM/λ_Noir and σ²_n/⟨n⟩.
%   • Fig 4: stray data tips cleared before saving.
%   • Fig 6: sparser legend (every ~8th φ shown).
%   • Fig 7 new: four-independent-witness panel (λ, T_c/T_w, σ²_n, τ_E).
%  ---------------------------------------------------------------------
%  Theory:
%     λ(φ) = d / [ 6 φ χ(φ) √(1-e_n²) ]              (Noirhomme 2021)
%     τ_λ = λ / v_T                                   (macroscopic)
%     τ_E = √π d / (24 φ χ v_T)                       (Enskog)
%     Clustering criterion:  λ(φ_c) = L/2  →  φ_c
% =====================================================================
...
fprintf('  ξ_smooth (Noirhomme, USED)  = %.4f\n', xi_smooth);
fprintf('  ξ_rot                       = %.4f\n', xi_rot);
fprintf('  ξ_full = ξ_smooth + ξ_rot   = %.4f  (for comparison)\n', xi_full);
fprintf('==========================================\n\n');
...
% Fit configuration
% ---------------------------------------------------------------------
% SATURATION HANDLING — choose how to treat very dilute systems where
% T(z) is nearly flat and λ → ∞:
%   'lower_bound'  (Option B): moderate upper bound; flag fits above
%                  lambda_max_phys as physical lower bounds; in Fig 1
%                  these appear as upward triangles ▲ with ↑ arrows.
%   'extend_bound' (Option A): raise upper bound to a very large value
%                  (lambda_upper_extended) so fits return huge but
%                  finite λ; all points plotted as normal circles.
% ---------------------------------------------------------------------
saturation_handling = 'extend_bound';   % <-- TOGGLE HERE: 'lower_bound' or 'extend_bound'

lambda_upper_bound   = 500;            % numerical upper bound (Option B)
lambda_upper_extended= 5000;           % numerical upper bound (Option A)
lambda_max_phys      = 20;             % saturation flag threshold (Option B only)
sigma_thresh         = 2.0;            % σ²_n trigger for clustering refit
Tratio_thresh        = 0.3;            % T_c/T_w trigger for clustering refit
outer_T_frac         = 0.10;           % keep shells where T > 0.10*T_outermost
...
R.fit_mode         = zeros(n_data,1);   % 1=full, 2=outer-only, 3=polyfit
...
for i = 1:n_data
    ...
    T_wall_outer = T_prof(idxs(end));     % outermost-shell T
    T_cent_inner = T_prof(idxs(1));       % innermost-shell T
    R.Tc_over_Tw(i) = T_cent_inner / T_wall_outer;
    
    % ===== STAGE 1: full fit =====
    z_full = dist_from_wall(idxs)';
    T_full = T_prof(idxs);
    [lam1, Tw1, ci1, ok1] = do_exp_fit(z_full, T_full, ...
    ...
    % Only flag saturation in 'lower_bound' mode; in 'extend_bound' mode
    % all fits return finite λ values that we plot directly.
    ...
    fprintf(' λ=%6.3f%s  T_c/T_w=%.3f  σ²/⟨n⟩=%5.2f  mode:%s\n', ...
        R.lambda_DEM(i), sat_tag, R.Tc_over_Tw(i), R.sigma2_n(i), ...
        mode_tag{max(R.fit_mode(i),1)});
end
...
valid_m_full = R.fit_mode == 1 & ~R.lambda_is_lb;
valid_m_outer= R.fit_mode == 2;
valid_m_lb   = R.lambda_is_lb;
valid_t      = R.tau_lambda > 0 & isfinite(R.tau_lambda) & ~R.lambda_is_lb;
...
plot_helper(phi_data, R.lambda_DEM, R.lambda_DEM_CI, valid_m_full, ...
            'o', col_dem);
plot_helper(phi_data, R.lambda_DEM, R.lambda_DEM_CI, valid_m_outer, ...
            's', col_dem2);
...
% Y-axis range adapts to saturation mode
if use_lower_bound_markers
    yl = [1e-2 1e2];
else
    yl = [1e-2 1e4];   % Option A: λ can reach ~1000 for very dilute fits
end
...
xlabel('$\phi$', 'Interpreter','latex','FontSize',28);
ylabel('$\lambda$ [cm]', 'Interpreter','latex','FontSize',28);

% Build legend entries adaptively (skip lower-bound entry in Option A)
leg_entries = { ...
    'Theory: $\lambda=d/[6\phi\chi\sqrt{1-e_n^2}]$', ...
    sprintf('Theory: $\\xi_{\\rm full}=%.2f$ (with rotation)', xi_full), ...
    'DEM (full fit)', ...
    'DEM (outer-only refit, clustering)'};
if use_lower_bound_markers && any(valid_m_lb)
    leg_entries{end+1} = sprintf('DEM lower bound ($\\lambda > %d$ cm)', lambda_max_phys);
end
legend(leg_entries, 'Interpreter','latex','FontSize',24,'Location','southwest');
legend boxoff;
set(gca,'FontSize',24,'FontName',fnt,'LineWidth',2, ...
...
xlabel('$\phi$', 'Interpreter','latex','FontSize',28);
ylabel('time [s]', 'Interpreter','latex','FontSize',28);
legend({ ...
    '$\tau_\lambda = \lambda/v_T$ (theory)', ...
    '$\tau_E = \sqrt{\pi}d/(24\phi\chi v_T)$ (theory)', ...
    '$\tau_\lambda$ from DEM', ...
    '$\tau_E$ from DEM'}, ...
    'Interpreter','latex','FontSize',22,'Location','southwest');
legend boxoff;
set(gca,'FontSize',28,'FontName',fnt,'LineWidth',2, ...
    'TickDir','in','TickLength',[0.018 0.018], ...
    'XMinorTick','on','YMinorTick','on');
xlim([min(phi_th) max(phi_th)]); box on;

%% ---------- Figure 3 (NEW): Coincident theory breakdown ===============
% λ_DEM/λ_Noir (length-scale breakdown) and σ²_n/⟨n⟩ (clustering order
% parameter) diverge from gas-phase values at the SAME φ_c.
...
ylabel('$\lambda_{\rm DEM}/\lambda_{\rm theory}$', ...
       'Interpreter','latex','FontSize',28,'Color',col_dem);
...
set(gca,'XScale','log','FontSize',28,'FontName',fnt,'LineWidth',1.8, ...
    'TickDir','in','XMinorTick','on','YMinorTick','on');
xlabel('$\phi$', 'Interpreter','latex','FontSize',28);
ylabel('$e_n$', 'Interpreter','latex','FontSize',28);
xlim([min(phi_g) max(phi_g)]); ylim([min(en_g) max(en_g)]);
box on;

% Clear stray data tips before saving
...
xlabel('$\phi$', 'Interpreter','latex','FontSize',28);
ylabel('$\sigma_n^2 / \langle n \rangle$', 'Interpreter','latex','FontSize',28);
...
xlabel('Distance from wall $z$ [cm]', 'Interpreter','latex','FontSize',28);
ylabel('$T(z) / T_{\rm wall}$', 'Interpreter','latex','FontSize',28);
yline(1, ':', 'Color', col_grid, 'LineWidth', 1.5, 'HandleVisibility','off');
legend('Interpreter','latex','FontSize',20,'Location','northeast');
legend boxoff;
set(gca,'FontSize',28,'FontName',fnt,'LineWidth',1.8, ...
    'TickDir','in','XMinorTick','on','YMinorTick','on');
ylim([0 1.2]); xlim([0 half_L]); box on;

%% ---------- Figure 7 (NEW): Four-witness panel ========================
...
plot_helper(phi_data, R.lambda_DEM, R.lambda_DEM_CI, valid_m_full, 'o', col_dem);
plot_helper(phi_data, R.lambda_DEM, R.lambda_DEM_CI, valid_m_outer, 's', col_dem2);
if any(valid_m_lb)
...
xline(phi_c_Noir, '-.', 'Color', [0.4 0.4 0.4], 'LineWidth', 1.6);
ylabel('$T_{\rm center}/T_{\rm wall}$', 'Interpreter','latex','FontSize',28);
...
set(gca,'FontSize',28,'FontName',fnt,'LineWidth',1.5, 'TickDir','in', ...
    'XMinorTick','on','YMinorTick','on');
...
fprintf('\n=========== Summary ===========\n');
fprintf('  φ      T_global   T_wall    λ_DEM   λ_Noir  fit    T_c/T_w  σ²/⟨n⟩\n');
fprintf('-------------------------------------------------------------------\n');
...
for i = 1:n_data
    ...
    fprintf('%.4f  %.3e  %.3e  %6.2f%s  %5.2f   %s  %6.3f   %6.2f\n', ...
        phi_data(i), R.T_global(i), R.T_wall_fit(i), R.lambda_DEM(i), sat, ...
        lam_th_i, mode_str{fm}, R.Tc_over_Tw(i), R.sigma2_n(i));
end
fprintf('-------------------------------------------------------------------\n');
fprintf('* = λ saturated (lower bound only)\n\n');

save_path = fullfile(pos_vel_dir, 'Phase_Diagram_Analysis_v2.mat');
save(save_path, 'R', 'phi_data', 'phi_th', 'lambda_Noir', 'lambda_full', ...
     'lambda_Noir_at_DEM', 'tau_lambda_th', 'tau_E_th', ...
     'phi_c_Noir', 'phi_c_full', 'xi_smooth', 'xi_full', ...
     'lambda_max_phys', 'sigma_thresh', 'Tratio_thresh', ...
     'saturation_handling', 'lam_fit_upper', '-v7.3');
fprintf('Results saved to: %s\n', save_path);

% Optional auto-save figures (uncomment to use)
% print(fig1, '-dpdf','-painters','-bestfit','Fig1_Lambda_vs_phi.pdf');
% print(fig2, '-dpdf','-painters','-bestfit','Fig2_Tau_vs_phi.pdf');
% print(fig3, '-dpdf','-painters','-bestfit','Fig3_Breakdown.pdf');
% print(fig4, '-dpdf','-painters','-bestfit','Fig4_Predictive_Phase_Map.pdf');
% print(fig5, '-dpdf','-painters','-bestfit','Fig5_Order_Param.pdf');
% print(fig6, '-dpdf','-painters','-bestfit','Fig6_Tz_Profiles.pdf');
% print(fig7, '-dpdf','-painters','-bestfit','Fig7_Four_Witnesses.pdf');

fprintf('\n✅ Done! 7 figures produced.\n');


%% ========== LOCAL FUNCTIONS (must be at end of script) ===============
function [lam, T_w, ci, ok] = do_exp_fit(z_, T_, lam_upper, T_upper)
% Fit T(z) = a * exp(-z/b) and return b=λ, a=T_wall, 95% CI on b, ok flag.
    try
        ...
        lam = fr.b;
        T_w = fr.a;
        ci  = ci_all(:,2)';   % CI on b (λ)
        ok  = isfinite(lam) && lam > 0;
    catch
        lam = NaN; T_w = NaN; ci = [NaN NaN]; ok = false;
    end
end

function plot_helper(phi_v, lam_v, lci_v, mask, marker, faceColor)
% Errorbar-plot a class of DEM points with CI handling and clipping.
